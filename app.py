...
from urllib.parse import urlparse  # <-- Nueva importación para el validador de URL
...
# Función personalizada de validación de URL
def is_safe_url(url):
    """Verifica si una URL es segura para redirección (mismo dominio)"""
    if not url:
        return False
    parsed_url = urlparse(url)
    return not parsed_url.netloc or parsed_url.netloc == request.host  # Compara con el host actual
...
system_prompt = {
    ...
    "content": (
        ...
        "Utiliza el historial de conversaciones si está disponible para adaptar la explicación al alumno. "
        "Todas las ecuaciones deben estar en formato LaTeX."
    )
}
